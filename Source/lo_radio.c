...
/*
**
** $Id: lo_radio.c,v 1.25 2003/07/06 16:51:33 olli Exp $
**
*/
...
static int doset( struct Data *data, APTR cl, APTR obj, struct TagItem *tags )
{
	struct TagItem *tag;
	int redraw = FALSE;
	...
	while( ( tag = NextTagItem( &tags ) ) ) switch( (int)tag->ti_Tag )
	{
		...
		case (int)MUIA_Selected:
			if( tag->ti_Data )
			{
				DoMethod( data->ctx->body, MM_Layout_FormRadio_SetActive, data->formobject, data->name, obj );
			}
			if( data->ix_onclick )
			{
...
DECCONST
{
	...
	doset( data, cl, obj, msg->ops_AttrList );
...
DECSET
{
	...
	if( doset( data, cl, obj, msg->ops_AttrList ) )
...
DECSMETHOD( Layout_FormRadio_SetActive )
{
	GETDATA;

	if( msg->form == data->formobject )
	{
		if( !stricmp( msg->name, data->name ) )
		{
			if( msg->obj != obj )
			{
...
case MM_Layout_FormRadio_SetActive: { return(handleMM_Layout_FormRadio_SetActive(cl, obj, (APTR)msg)); }
...
int create_loradioclass( void )
{
	...
	if( MUIMasterBase->lib_Version >= 20 )
		lcc->mcc_Class->cl_ID = "loradioClass";
#endif
